package net.sn0wix_.justEnoughKeybinds.util;
...
import net.sn0wix_.justEnoughKeybinds.gui.AdvancedConfirmScreen;
import net.sn0wix_.justEnoughKeybinds.gui.ParentScreenBlConsumer;
import net.sn0wix_.justEnoughKeybinds.keybinds.ChatKeys;
import net.sn0wix_.justEnoughKeybinds.keybinds.F3ShortcutsKeys;
...
public class Utils {
    ...
    public static Screen getModConfirmScreen(ParentScreenBlConsumer consumer, Text text) {
        return new AdvancedConfirmScreen(consumer, Text.empty(), text,
                Text.translatable("text.just-enough-keybinds.confirm." + new Random().nextInt(4)),
                Text.translatable("text.just-enough-keybinds.cancel." + new Random().nextInt(4)));
    }
    ...
    public static String nextValue(String errorRegex, BufferedReader reader) {
        try {
            ...
            JustEnoughKeybinds.LOGGER.error("Incorrect " + errorRegex + " format");
        }
    ...
    public static void showToastNotification(Text description) {
        MinecraftClient.getInstance().getToastManager().add(new SystemToast(new SystemToast.Type(2769), Text.literal(JustEnoughKeybinds.MOD_NAME), description));
    }

    public static void showToastNotification(Text description, long displayDuration) {
        MinecraftClient.getInstance().getToastManager().add(new SystemToast(new SystemToast.Type(displayDuration), Text.literal(JustEnoughKeybinds.MOD_NAME), description));
    }


    public static void rebindOldModDebugKeys() throws FileNotFoundException {
        File configFile = new File("config/" + JustEnoughKeybinds.MOD_ID + "/debug_keys.json");
        ...
        if (configFile.exists()) {
            ...
            debugKeysConfig.debugKeybindings.forEach((key, entry) -> {
                ...
                KeyBinding debugBinding = switch (key) {
                    case "key.just-enough-keybinds.show_hitboxes" -> options.debugShowHitboxesKey;
                    case "key.just-enough-keybinds.advanced_tooltips" -> options.debugShowAdvancedTooltipsKey;
                    case "key.just-enough-keybinds.creative_spectator" -> options.debugSpectateKey;
                    case "key.just-enough-keybinds.reload_chunks" -> options.debugReloadChunkKey;
                    case "key.just-enough-keybinds.chunk_boundaries" -> options.debugShowChunkBordersKey;
                    case "key.just-enough-keybinds.gamemodes" -> options.debugSwitchGameModeKey;
                    case "key.just-enough-keybinds.clear_chat" -> options.debugClearChatKey;
                    case "key.just-enough-keybinds.profiling" -> options.debugProfilingKey;
                    case "key.just-enough-keybinds.pause_focus" -> options.debugFocusPauseKey;
                    case "key.just-enough-keybinds.copy_location" -> options.debugCopyLocationKey;
                    case "key.just-enough-keybinds.dump_dynamic_textures" -> options.debugDumpDynamicTexturesKey;
                    case "key.just-enough-keybinds.inspect" -> options.debugCopyRecreateCommandKey;
                    case "key.just-enough-keybinds.reload_resourcepacks" -> options.debugReloadResourcePacksKey;
                    default -> null;
                };
